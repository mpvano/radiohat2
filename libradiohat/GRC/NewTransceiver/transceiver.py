...
import time
import threading
import transceiver_epy_block_0 as epy_block_0  # embedded python block
...
class transceiver(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        gr.top_block.__init__(self, "Transceiver", catch_exceptions=True)
        ...
        # Variables
        ##################################################
        self.txbalance = txbalance = 0.02
        self.samp_rate = samp_rate = 48000
        self.rxbalance = rxbalance = .03
        ...
        self.HPF = HPF = 2750
        self.Gain = Gain = 2
        self.CWLevel = CWLevel = 0.25
        self.CWFreq = CWFreq = 700
        self.CW = CW = 0
        self.ALSA = ALSA = 0
        self.ADCgain = ADCgain = 50
        ...
        self.top_grid_layout.addWidget(_PTT_check_box, 3, 2, 1, 1)
        for r in range(3, 4):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 3):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._txbalance_range = Range(-1, 1, .001, 0.02, 200)
        self._txbalance_win = RangeWidget(self._txbalance_range, self.set_txbalance, "balance", "counter", float, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_1.addWidget(self._txbalance_win, 3, 2, 1, 1)
        for r in range(3, 4):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(2, 3):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        self._rxbalance_range = Range(-1, 1, .001, .03, 1)
        self._rxbalance_win = RangeWidget(self._rxbalance_range, self.set_rxbalance, "balance", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._Volume_win = RangeWidget(self._Volume_range, self.set_Volume, "Volume", "dial", float, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_0.addWidget(self._Volume_win, 0, 2, 1, 1)
        for r in range(0, 1):
            self.RXTXTabs_grid_layout_0.setRowStretch(r, 1)
        for c in range(2, 3):
            self.RXTXTabs_grid_layout_0.setColumnStretch(c, 1)
        def _VSWR_probe():
          while True:
            ...
            val = self.epy_block_0.readVSWR()
            try:
              try:
                self.doc.add_next_tick_callback(functools.partial(self.set_VSWR,val))
              except AttributeError:
                self.set_VSWR(val)
            except AttributeError:
              pass
            time.sleep(1.0 / (5 if PTT else 1))
        _VSWR_thread = threading.Thread(target=_VSWR_probe)
        _VSWR_thread.daemon = True
        _VSWR_thread.start()
        self._VFO_range = Range(3500, 32000000, 100, 7074000, 3000)
        self._VFO_win = RangeWidget(self._VFO_range, self.set_VFO, "VFO", "counter_slider", int, QtCore.Qt.Horizontal)
        ...
        self._ToneLevel1_win = RangeWidget(self._ToneLevel1_range, self.set_ToneLevel1, "Level 1", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._ToneLevel_win = RangeWidget(self._ToneLevel_range, self.set_ToneLevel, "Level 0", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._ToneFreq1_win = RangeWidget(self._ToneFreq1_range, self.set_ToneFreq1, "Freq 1", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._ToneFreq_win = RangeWidget(self._ToneFreq_range, self.set_ToneFreq, "Freq 0", "counter", float, QtCore.Qt.Horizontal)
        ...
        def _Power_probe():
          while True:
            ...
            val = self.epy_block_0.readForward()
            try:
              try:
                self.doc.add_next_tick_callback(functools.partial(self.set_Power,val))
              except AttributeError:
                self.set_Power(val)
            except AttributeError:
              pass
            time.sleep(1.0 / (5 if PTT else 1))
        _Power_thread = threading.Thread(target=_Power_probe)
        _Power_thread.daemon = True
        _Power_thread.start()
        ...
        self.RXTXTabs_grid_layout_1.addWidget(_MuteMic_check_box, 2, 3, 1, 1)
        for r in range(2, 3):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(3, 4):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        _LSB_check_box = Qt.QCheckBox("LSB")
        ...
        self.top_grid_layout.addWidget(_LSB_check_box, 3, 0, 1, 1)
        for r in range(3, 4):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._LPF_range = Range(50, 850, 10, 50, 200)
        self._LPF_win = RangeWidget(self._LPF_range, self.set_LPF, "High pass", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._HPF_win = RangeWidget(self._HPF_range, self.set_HPF, "Low pass", "counter", float, QtCore.Qt.Horizontal)
        ...
        self._Gain_win = RangeWidget(self._Gain_range, self.set_Gain, "Gain", "dial", float, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_0.addWidget(self._Gain_win, 0, 1, 1, 1)
        for r in range(0, 1):
            self.RXTXTabs_grid_layout_0.setRowStretch(r, 1)
        for c in range(1, 2):
            self.RXTXTabs_grid_layout_0.setColumnStretch(c, 1)
        self._CWLevel_range = Range(0, 1, .01, 0.25, 200)
        self._CWLevel_win = RangeWidget(self._CWLevel_range, self.set_CWLevel, "CW Level", "counter", float, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_1.addWidget(self._CWLevel_win, 3, 1, 1, 1)
        for r in range(3, 4):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(1, 2):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        self._CWFreq_range = Range(10, 10000, 10, 700, 200)
        self._CWFreq_win = RangeWidget(self._CWFreq_range, self.set_CWFreq, "CW Offset", "counter", float, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_1.addWidget(self._CWFreq_win, 3, 0, 1, 1)
        for r in range(3, 4):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(0, 1):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(_CW_check_box, 3, 1, 1, 1)
        for r in range(3, 4):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(1, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.RXTXTabs_grid_layout_1.addWidget(_ALSA_check_box, 1, 3, 1, 1)
        for r in range(1, 2):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(3, 4):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        self._ADCgain_range = Range(0, 100, 1, 50, 200)
        self._ADCgain_win = RangeWidget(self._ADCgain_range, self.set_ADCgain, "ADCgain", "dial", int, QtCore.Qt.Horizontal)
        self.RXTXTabs_grid_layout_0.addWidget(self._ADCgain_win, 0, 0, 1, 1)
        for r in range(0, 1):
            self.RXTXTabs_grid_layout_0.setRowStretch(r, 1)
        for c in range(0, 1):
            self.RXTXTabs_grid_layout_0.setColumnStretch(c, 1)
        ...
        self.qtgui_time_sink_x_0.set_y_label("TxAudio", "")
        ...
        self.qtgui_number_sink_0.set_update_time(0.2 if PTT else 1)
        ...
        self.RXTXTabs_grid_layout_1.addWidget(self._qtgui_number_sink_0_win, 0, 2, 1, 1)
        for r in range(0, 1):
            self.RXTXTabs_grid_layout_1.setRowStretch(r, 1)
        for c in range(2, 3):
            self.RXTXTabs_grid_layout_1.setColumnStretch(c, 1)
        ...
        self.qtgui_freq_sink_x_0.set_y_axis((-110), (-40))
        ...
        self.blocks_multiply_const_vxx_4 = blocks.multiply_const_ff(1 if PTT else 0)
        self.blocks_multiply_const_vxx_3 = blocks.multiply_const_ff(TXScale)
        self.blocks_multiply_const_vxx_2_0 = blocks.multiply_const_ff(0.1 if (PTT and CW) else 0)
        self.blocks_multiply_const_vxx_2 = blocks.multiply_const_ff(0 if PTT else 1)
        ...
        self.blocks_message_strobe_1 = blocks.message_strobe(pmt.from_long(ADCgain), 50)
        self.blocks_message_strobe_0_0_0 = blocks.message_strobe(pmt.from_long(CW), 50)
        self.blocks_message_strobe_0_0 = blocks.message_strobe(pmt.from_long(PTT), 2 if CW else 10)
        ...
        self.blocks_add_const_vxx_1 = blocks.add_const_ff(VSWR)
        self.blocks_add_const_vxx_0 = blocks.add_const_ff(Power)
        ...
        self.analog_sig_source_x_1 = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, CWFreq, CWLevel if (PTT and CW) else 0, 0, 0)
        self.analog_sig_source_x_0_0 = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, ToneFreq1, (0 if CW else (ToneLevel1 * TXTone1)), 0, 0)
        self.analog_sig_source_x_0 = analog.sig_source_f(samp_rate, analog.GR_COS_WAVE, ToneFreq, (0 if CW else (ToneLevel * TXTone)), 0, 0)
        ...
        self.msg_connect((self.blocks_message_strobe_0_0_0, 'strobe'), (self.epy_block_0, 'cwmode'))
        self.msg_connect((self.blocks_message_strobe_1, 'strobe'), (self.epy_block_0, 'adc'))
        self.connect((self.analog_agc2_xx_0, 0), (self.blocks_add_xx_2_0, 1))
        ...
        self.connect((self.analog_sig_source_x_1, 0), (self.blocks_add_xx_1, 3))
        ...
        self.connect((self.blocks_add_const_vxx_1, 0), (self.qtgui_number_sink_0, 1))
        self.connect((self.blocks_add_xx_1, 0), (self.blocks_multiply_const_vxx_3, 0))
        self.connect((self.blocks_add_xx_2, 0), (self.audio_sink_0, 0))
        self.connect((self.blocks_add_xx_2_0, 0), (self.audio_sink_1, 0))
        self.connect((self.blocks_add_xx_3, 0), (self.audio_sink_0, 1))
        self.connect((self.blocks_add_xx_3, 0), (self.blocks_multiply_const_vxx_4, 0))
        ...
        self.connect((self.blocks_multiply_const_vxx_2_0, 0), (self.blocks_add_xx_2_0, 0))
        self.connect((self.blocks_multiply_const_vxx_3, 0), (self.ssb_mod_0, 0))
        self.connect((self.blocks_multiply_const_vxx_4, 0), (self.qtgui_time_sink_x_0, 0))
        self.connect((self.blocks_null_source_0, 0), (self.blocks_add_const_vxx_0, 0))
        self.connect((self.blocks_null_source_0, 0), (self.blocks_add_const_vxx_1, 0))
    ...
    def set_samp_rate(self, samp_rate):
        ...
        self.band_pass_filter_0.set_taps(firdes.band_pass((self.Gain * (1-self.PTT)), self.samp_rate, 400 if self.CW else self.LPF, 900 if self.CW else self.HPF, 100, window.WIN_BLACKMAN, 6.76))
    ...
    def set_VSWR(self, VSWR):
        self.VSWR = VSWR
        self.blocks_add_const_vxx_1.set_k(self.VSWR)
    ...
    def set_ToneLevel1(self, ToneLevel1):
        ...
        self.analog_sig_source_x_0_0.set_amplitude((0 if self.CW else (self.ToneLevel1 * self.TXTone1)))
    ...
    def set_ToneLevel(self, ToneLevel):
        ...
        self.analog_sig_source_x_0.set_amplitude((0 if self.CW else (self.ToneLevel * self.TXTone)))
    ...
    def get_TXScale(self):
        return self.TXScale

    def set_TXScale(self, TXScale):
        self.TXScale = TXScale
        self.blocks_multiply_const_vxx_3.set_k(self.TXScale)
    ...
    def set_Power(self, Power):
        self.Power = Power
        self.blocks_add_const_vxx_0.set_k(self.Power)
    ...
    def set_PTT(self, PTT):
        self.PTT = PTT
        self._PTT_callback(self.PTT)
        self.analog_sig_source_x_1.set_amplitude(self.CWLevel if (self.PTT and self.CW) else 0)
        self.band_pass_filter_0.set_taps(firdes.band_pass((self.Gain * (1-self.PTT)), self.samp_rate, 400 if self.CW else self.LPF, 900 if self.CW else self.HPF, 100, window.WIN_BLACKMAN, 6.76))
        self.blocks_message_strobe_0_0.set_msg(pmt.from_long(self.PTT))
        self.blocks_multiply_const_vxx_2.set_k(0 if self.PTT else 1)
        self.blocks_multiply_const_vxx_2_0.set_k(0.1 if (self.PTT and self.CW) else 0)
        self.blocks_multiply_const_vxx_4.set_k(1 if self.PTT else 0)
        self.mic_preamp_1.set_PTT(self.PTT)
        self.qtgui_number_sink_0.set_update_time(0.2 if self.PTT else 1)
    ...
    def set_MuteMic(self, MuteMic):
        ...
        self._MuteMic_callback(self.MuteMic)
        self.mic_preamp_1.set_MuteMic(self.MuteMic or self.CW)
    ...
    def get_CWLevel(self):
        return self.CWLevel

    def set_CWLevel(self, CWLevel):
        self.CWLevel = CWLevel
        self.analog_sig_source_x_1.set_amplitude(self.CWLevel if (self.PTT and self.CW) else 0)
    ...
    def set_CWFreq(self, CWFreq):
        self.CWFreq = CWFreq
        self.analog_sig_source_x_1.set_frequency(self.CWFreq)
    ...
    def set_CW(self, CW):
        ...
        self.analog_sig_source_x_1.set_amplitude(self.CWLevel if (self.PTT and self.CW) else 0)
        self.band_pass_filter_0.set_taps(firdes.band_pass((self.Gain * (1-self.PTT)), self.samp_rate, 400 if self.CW else self.LPF, 900 if self.CW else self.HPF, 100, window.WIN_BLACKMAN, 6.76))
        self.blocks_message_strobe_0_0.set_period(2 if self.CW else 10)
        self.blocks_message_strobe_0_0_0.set_msg(pmt.from_long(self.CW))
        self.blocks_multiply_const_vxx_2_0.set_k(0.1 if (self.PTT and self.CW) else 0)
        self.mic_preamp_1.set_MuteMic(self.MuteMic or self.CW)
    ...
    def get_ADCgain(self):
        return self.ADCgain

    def set_ADCgain(self, ADCgain):
        self.ADCgain = ADCgain
        self.blocks_message_strobe_1.set_msg(pmt.from_long(self.ADCgain))
