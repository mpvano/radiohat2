...
#ifdef RASPBERRY_PI_I2C
int si5351fd;
#endif

#ifdef SI5351A_B04486_GT
	i2c_addr = 0x62;		//	needed for certain preprogrammed parts
#endif
...
bool Si5351::init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	// Start I2C comms
   	uint8_t reg_val = 0;
   if (		((si5351fd = open("/dev/i2c-1", O_RDWR)) >= 0)
   		&&	(ioctl(si5351fd, I2C_SLAVE, i2c_bus_addr & 0xff) >= 0) )
   		reg_val = 0;
	else reg_val = -1;
#endif

#ifndef RASPBERRY_PI_I2C
	// Start I2C comms
	Wire.begin();
...
uint8_t buf[33];
	...
	if (bytes <= 31)
		...
		result = write(si5351fd, buf, bytes+1);
		if (result > 0)
			result--;
		}
	return result = 0x0ff;	
...
uint8_t buf[2];
	...
	int result = write(si5351fd, buf, 2);
	return (result < 0) ? 0 : 1;
#endif
}
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	//	caller seems to expect only last byte if more than one received (!)
	if ((write(si5351fd, buf, 1) == 1) && ((readcount = read(si5351fd, buf, 1)) > 0))
//		reg_val = buf[readcount-1];
		reg_val = buf[0];
	return reg_val;
